import type { PieceColor } from "../../components/ChessBoard/chessBoard.types"

interface KnightProps {
  color: PieceColor
  className?: string
}

function Knight({ color, className = "" }: KnightProps) {
  const fillColor = color === "white" ? "#f8f9fa" : "#343a40"
  const strokeColor = color === "white" ? "#343a40" : "#343a40"
  const eyeColor = color === "white" ? "#343a40" : "#f8f9fa"

  return (
    <svg
      viewBox="0 0 45 45"
      className={`chess-piece chess-piece--knight ${className}`}
      fill="none"
      fillRule="evenodd"
      stroke={strokeColor}
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth="1.5"
    >
      <path fill={fillColor} d="M22 10.3c10.5 1 16.5 8 16 29H15c0-9 10-6.5 8-21" />
      <path
        fill={fillColor}
        d="M24 18.3c.38 2.91-5.55 7.37-8 9-3 2-2.82 4.34-5 4-1.042-.94 1.41-3.04 0-3-1 0 .19 1.23-1 2-1 0-4.003 1-4-4 0-2 6-12 6-12s1.89-1.9 2-3.5c-.73-.994-.5-2-.5-3 1-1 3 2.5 3 2.5h2s.78-1.992 2.5-3c1 0 1 3 1 3"
      />
      <path fill={eyeColor} stroke={eyeColor} d="M9.5 25.8a.5.5 0 1 1-1 0 .5.5 0 1 1 1 0" />
      <path fill={eyeColor} stroke={eyeColor} strokeWidth="1.49997" d="M14.933 16.05a.5 1.5 30 1 1-.866-.5.5 1.5 30 1 1 .866.5" />
      <path
        fill={eyeColor}
        stroke="none"
        d="m24.55 10.7-.45 1.45.5.15c3.15 1 5.65 2.49 7.9 6.75s3.25 10.31 2.75 20.25l-.05.5h2.25l.05-.5c.5-10.06-.88-16.85-3.25-21.34s-5.79-6.64-9.19-7.16z"
      />
    </svg>
  )
}

export default Knight
